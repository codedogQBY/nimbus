...
// 从快照数据中获取所有文件
async function getAllFilesFromSnapshot(
  contents: any,
  basePath: string = "",
): Promise<Array<{ file: any; relativePath: string }>> {
  ...
  if (contents.files) {
    ...
    const storageSourceMap = new Map(
      storageSources.map((source: any) => [source.id, source]),
    );

    files = contents.files.map((file: any) => ({
      file: {
        ...file,
        storageSource: storageSourceMap.get(file.storageSourceId) || {
          type: "local",
          config: {},
        },
      },
  ...
  try {
    ...
    const readableStream = new ReadableStream({
      start(controller) {
        ...
        // 添加文件到压缩包
        addFilesToArchive(archive, allFiles)
          .then(() => {
            archive.finalize();
          })
          .catch((err) => {
...
async function addFilesToArchive(
  ...
  allFiles: Array<{ file: any; relativePath: string }>,
) {
