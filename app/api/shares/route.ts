  ...
  try {
    ...
    const formattedShares = shares.map((share: any) => {
      ...
      return {
        id: share.id,
        name: snapshotData
          ? snapshotData.name || snapshotData.originalName || "未知"
          : "未知",
        ...
        // 添加额外的元数据
        size:
          shareType === "file" ? snapshotData?.size : snapshotData?.totalSize,
        mimeType: shareType === "file" ? snapshotData?.mimeType : undefined,
        totalFiles:
          shareType === "folder" ? snapshotData?.totalFiles : undefined,
        totalFolders:
          shareType === "folder" ? snapshotData?.totalFolders : undefined,
      };
    });
    ...
    const stats = {
      total: shares.length,
      totalViews: shares.reduce(
        (sum: number, s: any) => sum + (s.viewCount || 0),
        0,
      ),
      totalDownloads: shares.reduce(
        (sum: number, s: any) => sum + s.downloadCount,
        0,
      ),
