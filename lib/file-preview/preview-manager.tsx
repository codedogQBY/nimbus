...
// 预览图标组件 - 更美观的版本
export function getPreviewIcon(filename: string, mimeType?: string): React.ReactNode {
...
// 获取预览状态信息
export function getPreviewStatus(filename: string, mimeType?: string): {
  supported: boolean;
  type: PreviewType;
  ...
  if (supported) {
    if ([PreviewType.IMAGE, PreviewType.VIDEO, PreviewType.AUDIO, PreviewType.PDF].includes(previewType)) {
      status = 'ready';
    } else {
      status = 'coming-soon';
    }
